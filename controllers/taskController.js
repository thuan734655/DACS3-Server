...
export const updateTask = async (req, res) => {
  try {
    ...
    // Check if user has permission in this workspace
    const hasPermission = await checkWorkspacePermission(task.workspace_id, userId);
    if (!hasPermission) {
      return res.status(403).json({
        success: false,
    ...
    const assignmentChanged = assigned_to && task.assigned_to && 
                            assigned_to.toString() !== task.assigned_to.toString();
    const previousAssignee = task.assigned_to;
    
    // Keep track of whether epic changed
    const epicChanged = epic_id && task.epic_id && 
                      epic_id.toString() !== task.epic_id.toString();
    ...
    if (status === "Done" && task.status !== "Done") {
      ...
      const managementMembers = workspace.members.filter(
        member => ['Leader', 'Manager'].includes(member.role)
      );
      ...
      if (task.created_by.toString() !== userId && 
        ...
        io.to(`user-${task.created_by}`).emit('notification:new', await creatorNotification.populate("user_id workspace_id"));
      }
    }
...
export const addComment = async (req, res) => {
  try {
    ...
    if (task.assigned_to && task.assigned_to.toString() !== userId) {
      ...
      await assigneeNotification.save();
      io.to(`user-${task.assigned_to}`).emit('notification:new', await assigneeNotification.populate("user_id workspace_id"));
    }
    
    // Notify creator if they aren't the commenter or assignee
    if (task.created_by.toString() !== userId && 
        (!task.assigned_to || task.created_by.toString() !== task.assigned_to.toString())) {
      const creatorNotification = new Notification({
        user_id: task.created_by,
        type: "task_comment",
        type_id: taskId,
