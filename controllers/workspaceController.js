...
export const updateWorkspace = async (req, res) => {
  try {
    ...
    const notificationPromises = membersToNotify.map(async (member) => {
      ...
      io.to(`user-${member.user_id._id}`).emit(
        "notification:new",
  ...
  try {
    ...
    if (workspace.created_by.toString() === targetUserId) {
      return res.status(403).json({
        success: false,
        message:
          "Cannot remove the workspace leader. The leader should delete the workspace instead.",
      });
    }
    ...
    if (checkUserRole(workspace, userId, ["Manager"])) {
      const targetUser = workspace.members.find(
        (member) => member.user_id.toString() === targetUserId
      );
      if (
        targetUser &&
        (targetUser.role === "Leader" || targetUser.role === "Manager")
      ) {
        return res.status(403).json({
          success: false,
          message:
            "Managers cannot remove leaders or other managers from the workspace",
        });
      }
    }
    ...
    io.to(`user-${targetUserId}`).emit(
      "notification:new",
      await removalNotification.populate("user_id")
    );
...
export const joinWorkspace = async (req, res) => {
  try {
    ...
    const ownerNotification = new Notification({
      ...
      content: `${req.user.name || "A new user"} joined workspace "${
        workspace.name
      }"`,
      related_id: userId,
      is_read: false,
    ...
    io.to(`user-${workspace.created_by}`).emit(
      "notification:new",
      await ownerNotification.populate("user_id workspace_id")
    );
  ...
  try {
    ...
    if (workspace.created_by.toString() === userId) {
      return res.status(403).json({
        success: false,
        message:
          "The workspace creator cannot leave the workspace. Transfer ownership or delete the workspace instead.",
      });
    }
    ...
    const ownerNotification = new Notification({
      ...
      type_id: workspaceId,
      workspace_id: workspaceId,
      content: `${req.user.name || "A user"} left workspace "${
        workspace.name
      }"`,
      related_id: userId,
      is_read: false,
